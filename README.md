...
- 一条水池上的机械赛道：开放的玩具锤路段、窄桥、左右横移并完整让空的平台、两个检查点和终点。
- WASD / 方向键移动，空格刹车，R 重开，Esc 暂停或继续。
- 掉落从最近检查点重生，计时继续；切换标签页或失去窗口焦点自动暂停。
- 完赛结算、目标奖牌、每种玩法最快 20 次成绩；新版与旧版分开比较，旧纪录和设置保留。
...
- `src/state.ts`：游戏状态与 `marble-lab-v2` 分桶保存；`marble-lab-v1` 只读保留原文。

### 轨道外观资源

关卡配置与运行时代码已分离。当前仍为本地单关卡版本，场景布局不再写在运行时内。精细 GLB 仍是整关模型，布局变化不会自动更新外观；可将配置的 `visuals` 设为 `null`，用基础几何体验证碰撞。Blender 生成脚本尚未自动读取关卡配置。
