...
2026-09-14，依据[第四关设计任务](../product/level-04-design.md)及[正式开发任务](../product/two-courses-implementation.md)转入实施。独立id=`three-route`，目录编号04，规则`standard`，无奖牌阈值；与顶级难度单路线分别保留。正式白盒已交付，模型、目录接入及三路试玩尚待证据。
...
实施交付：[`three-route.ts`](../../src/levels/three-route.ts)、[精确施工JSON](three-route-construction.json)。原layout与俯视图保留设计中心线口径，施工JSON提供台面、固定结构、支撑与进度元数据；TS为运行配置。已恢复`three-route-track.glb`静态引用；ART已交付资源结构与哈希，运行装配及A/B/C完整试玩仍待CODE验证。已填有向组末gate和4CP顺序，gate不重生、不增加HUD检查点。必要测试、构建及隔离试玩由CODE统一执行，未据实完成前不宣称可通或验收通过。
...
按球半径.425、臂上半径3.5、横向留边.275估算，投影角带约±4.506°、时间约±.150秒。它只是对齐几何条件，不保证任意球速/横漂都能跨缝；要在离台前瞄准目标岸。岸上外向标记只作视觉引导；选择在到达该路入口CP时提交，在转台上尚未下岸前仍可改变主意。
...
在十字上、尚未外向越过岸上选路标记前可以自由改走位；进入支路入口CP后本局选路锁定。物理上不放透明门禁止返回，但返回/误入别路不会重写已选进度；要主动改选可重开本局。当前由新course协议承接有向gate、条件CP与路线状态；数据已填，锁路时机和掉落后保留仍需CODE实际验证。
...
本轮已正式派发CODE/ART实施：多实例/朝向/相位与course协议由CODE交付，LEVEL已填写两份TS及精确施工JSON；ART按施工数据制作静态资源。反向推墙、动态接缝、高层支撑、分支保持及完整通行均以实际装配和试玩证据验收。
