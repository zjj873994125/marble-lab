...
- **关卡对话**：维护 `src/levels/initial-gravity.ts`，关卡设计说明放在 `docs/levels/`。负责位置、尺寸、碰撞代理布局、检查点顺序、终点、掉落线、已有机关参数和进度分段；不编辑 `runtime.ts`、加载器、Vue 或模型源文件。
...
当前目录没有 Git 历史可用于区分手动修改。每次编辑前读取最新文件，在内存中记录内容或校验值，写入前再核对，不生成额外文件备份；发现用户或其他对话已修改相同内容，先说明原因及拟修改范围并获得用户确认。不要全文件格式化、批量修复或复制旧项目覆盖当前文件。
...
运行时和 HUD 读取同一份配置。当前只接入“教学关卡”这一关；增加一个配置文件不会自动出现在关卡选择页。当前 `rulesVersion` 区分 classic、open-hammer 与 flat-hammer，省略为 classic；`marble-lab-v2` 按各规则分桶，保留所有已有桶，v1 原文只读保留，旧成绩不会参与新版最佳。没有通用多关卡存档；后续改变真实规则前仍需由代码确认版本归属，不能清空记录。
...
- `pendulum`：当前固定一个沿 Z 方向运动的摆锤，`ball` 为当前玩具锤的 Z 向平端圆柱运动学代理（旧版球/胶囊成绩单独归档），`rod` 为独立锤柄，`anchor` 为杆上端。偏移为 `sin(time * angularSpeed) * amplitude`；Y 额外增加 `abs(偏移) * lift`；杆宽为 `rodWidth`。
...
- `progress`：第 n 项对应已通过 n 个检查点。公式为 `base + clamp((z - originZ) * direction / divisor, 0, max)`；`direction` 为 `-1` 或 `1`，`divisor > 0`。必须有“检查点数量 + 1”项；每项 `base + max <= 1`，完赛时设为 1。当前只支持沿 Z 轴估算，不是自动测算路径长度。
...
代码对话统一进行浏览器接入验证，复用 `http://127.0.0.1:5173/`，确认服务工作目录是本项目。布局或模型变更还要检查：加载、外观与碰撞对齐、开局、刹车、暂停、重开、机关运动、按序检查点、掉落重生和终点结算。需要写入测试成绩时使用隔离测试浏览器，不清空或覆盖用户当前浏览器的设置与成绩。

当前仍是一个静态场景、一个玩具锤、一个移动平台和顺序检查点，不是通用关卡编辑器。Vue 与运行时的 `start / setPhase / applySettings / destroy`、`tick / finish / pause / restart` 边界保留。PlayCanvas Editor 场景导入和多关卡管理尚未实现。
