...
状态：2026-09-11 恢复开发后的接口约定，已按三锤、十字旋转台及向 -X 坡道修订。新增类型、姿态计算及运行时已落地，3项机关姿态测试、资源生命周期与双关存档测试通过（当前统一测试23项），已通知 LEVEL 填写正式 TS；第二关注册和真实通关仍待配套验证。第一关文件与参数保持，新关卡文件建立后原位维护，不建版本副本或备份。
