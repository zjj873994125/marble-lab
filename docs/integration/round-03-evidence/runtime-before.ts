...
export async function createGame(canvas: HTMLCanvasElement, hooks: Hooks): Promise<MarbleGame> {
  ...
  const cream = mat('#fff9e7'), edge = mat('#c9d3ce'), orange = mat('#e77442'), dark = mat('#465d64'), blue = mat('#91b7c3'), floorMat = mat('#dce2d8'), steel = mat('#a0c4d1', .65, .8), lightMat = mat('#e4eded')
  function primitive(name: string, type: 'box' | 'sphere' | 'cylinder', pos: number[], scale: number[], material: pc.StandardMaterial, body?: 'static' | 'dynamic' | 'kinematic') {
    const e = new pc.Entity(name)
    e.addComponent('render', { type, material, castShadows: true, receiveShadows: true })
    e.setPosition(pos[0]!,pos[1]!,pos[2]!); e.setLocalScale(scale[0]!,scale[1]!,scale[2]!); app.root.addChild(e)
    if (body) {
  ...
  const palette = { cream, edge, orange, dark, blue, floorMat }
  const replaced: pc.RenderComponent[] = []
  function addObject(config: PrimitiveConfig, body: 'static' | 'kinematic' | undefined = config.body) {
    const entity = primitive(config.name, config.type, config.position, config.size, palette[config.material], body)
  ...
  const ball = primitive('Player marble','sphere',level.start.previewPosition,[.85,.85,.85],steel,'dynamic')
  const bandMesh = pc.Mesh.fromGeometry(app.graphicsDevice, new pc.TorusGeometry({ tubeRadius: .018, ringRadius: .5, segments: 40, sides: 8 })); meshes.push(bandMesh)
  const band = new pc.Entity('Marble seam'); band.addComponent('render',{ meshInstances: [new pc.MeshInstance(bandMesh,lightMat)] }); band.setLocalEulerAngles(58,0,0); ball.addChild(band)
  ...
  function applySettings() { app.graphicsDevice.maxPixelRatio=hooks.settings().quality==='high' ? Math.min(devicePixelRatio,1.7) : 1; sun.light!.castShadows=hooks.settings().quality==='high'; resize() }
  applySettings()
  app.on('update',(delta: number)=>{
    ...
    if (phase==='playing') {
      ...
      const direction = new pc.Vec3().sub2(b,a).normalize(); const rotation = new pc.Quat().setFromDirections(pc.Vec3.UP,direction); rod.setRotation(rotation)
      const z = platformConfig.centerZ+Math.sin(time*platformConfig.angularSpeed)*platformConfig.amplitude
      platform.setPosition(platformConfig.body.position[0],platformConfig.body.position[1],z); platformStripe.setPosition(platformConfig.stripe.position[0],platformConfig.stripeY,z)
  ...
  app.start()
  return { start,setPhase,applySettings,destroy() { observer.disconnect(); window.removeEventListener('keydown',down); window.removeEventListener('keyup',up); window.removeEventListener('blur',blur); document.removeEventListener('visibilitychange',hidden); void audio?.close(); disposeTrackVisuals(); app.destroy(); materials.forEach(m=>m.destroy()); meshes.forEach(m=>m.destroy()) } }
}
