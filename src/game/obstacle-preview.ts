...
export function createObstaclePreview(canvas:HTMLCanvasElement,id:ObstacleId,onStatus:(message:string)=>void,selection:()=>TrackThemeId=()=> 'classic',waterSettings:()=>Pick<Settings,'quality'|'reducedMotion'|'waterSpeed'|'ballSkin'>=()=>({quality:'high',reducedMotion:false,waterSpeed:1,ballSkin:'steel'})):ObstaclePreview {
  ...
  const controller=new AbortController(),assets=createModelAssets(app,controller.signal),library=createObstacleLibrary(assets.load,themes.applyEntity)
  ...
  catch(error){library.destroy();themes.destroy();controller.abort();assets.destroy();preparedWater?.destroy();app.destroy();materials.forEach(material=>material.destroy());throw error}
