...
export async function attachTrackVisuals(app: pc.Application, platform: pc.Entity | undefined, config: TrackVisualsConfig | null, replaced: pc.RenderComponent[], sharedLoad?: (file: string) => Promise<pc.Asset>): Promise<() => void> {
  if (!config) return () => {}
