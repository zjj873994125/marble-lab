...
const viewport = reactive({ width: innerWidth, height: innerHeight, touch: coarsePointer.matches })
...
function resizeViewport() { viewport.width = innerWidth; viewport.height = innerHeight; viewport.touch = coarsePointer.matches }
...
onMounted(() => { window.addEventListener('resize', resizeViewport); window.addEventListener('orientationchange', resizeViewport); visualViewport?.addEventListener('resize', resizeViewport); coarsePointer.addEventListener('change', resizeViewport) })
onBeforeUnmount(() => { setTouchInput(emptyInput()); window.removeEventListener('resize', resizeViewport); window.removeEventListener('orientationchange', resizeViewport); visualViewport?.removeEventListener('resize', resizeViewport); coarsePointer.removeEventListener('change', resizeViewport) })
...
<template>
  <main class="app-shell" :class="{ 'in-game': inGame, 'water-scene': hasWater, 'touch-layout': touchMode }">
    <GameCanvas :key="state.levelId" :input="touchInput" />
    ...
    <template v-else>
      <div class="game-top">
        ...
        <el-button class="pause-button" :icon="VideoPause" @click="state.phase = 'paused'">暂停 <kbd v-if="!touchMode">Esc</kbd></el-button>
      </div>
      <div class="progress-rail" aria-label="关卡进度"><span :style="{ width: `${state.progress * 100}%` }"/></div>
      <div v-if="!touchMode" class="game-bottom"><div class="control-hint"><span><kbd>W A S D</kbd> / <kbd>↑ ← ↓ →</kbd> 移动</span><span><kbd>Space</kbd> 刹车</span></div><el-button class="restart-button" :icon="RefreshRight" @click="startRun">重新挑战 <kbd>R</kbd></el-button></div>
      <TouchControls v-if="touchMode && state.phase === 'playing' && !portraitBlocked" :reset-key="inputResetKey" @change="setTouchInput" />
    </template>
    <footer v-if="!inGame" class="bottom-bar"><span><i class="status-dot"/>{{ state.error ? '场景加载失败' : state.ready ? '准备就绪' : '正在准备 3D 场景…' }}</span><span>{{ touchMode ? '横屏触控 · 支持键盘' : '键盘操作 · 推荐电脑浏览器' }}</span><span>LOCAL EDITION <b>01</b></span></footer>
    ...
    <el-dialog :model-value="state.phase === 'paused' && !state.settingsOpen && !portraitBlocked" title="稍作停留" width="420px" align-center :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false" class="game-dialog">
      <p class="dialog-subtitle">计时已暂停，下一段路等你出发。<span v-if="touchMode" class="mobile-pause-detail">{{ activeLevel.title }} · 掉落 {{ state.falls }} 次</span></p><div class="dialog-stack"><el-button type="primary" size="large" :icon="ArrowRight" @click="state.phase = 'playing'">继续挑战</el-button><el-button size="large" :icon="RefreshRight" @click="startRun">重新开始</el-button><el-button size="large" :icon="Setting" @click="settings">游戏设置</el-button><el-button text @click="state.phase = 'menu'">返回主界面</el-button></div>
    </el-dialog>
