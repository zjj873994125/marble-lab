...
<template>
  <main class="app-shell" :class="{ 'in-game': inGame, 'water-scene': hasWater, 'touch-layout': touchMode, 'atlas-open': inAtlas, 'home-menu': inHome }">
    <GameCanvas v-if="!inAtlas" :key="`${state.levelId}/${state.sceneLoadId}`" :input="touchInput" />
    <header v-if="!inGame" class="topbar">
      <RouterLink to="/" class="brand" aria-label="杰哥让你滚首页"><span class="brand-orbit"><i/></span><span>杰哥让你滚<small v-if="!inHome">MARBLE LAB</small></span></RouterLink>
      <nav aria-label="主导航"><RouterLink to="/" exact-active-class="active">开始</RouterLink><RouterLink to="/obstacles" exact-active-class="active">障碍图鉴</RouterLink><RouterLink to="/records" exact-active-class="active">我的纪录</RouterLink></nav>
      <div class="top-actions"><span v-if="inHome" class="home-ready-status" :class="{ failed: !!state.error, ready: state.ready && !state.error }" role="status"><i/>{{ state.error ? '加载失败' : state.ready ? '准备就绪' : '准备中' }}</span><el-button circle :icon="QuestionFilled" aria-label="操作说明" @click="state.helpOpen = true"/><el-button circle :icon="Setting" aria-label="设置" @click="settings"/></div>
    </header>
    <RouterView v-if="!inGame" />
    <template v-else>
      <button v-if="cleanHud && state.phase === 'playing'" type="button" class="clean-timer" :style="{ '--clean-viewport-x': `${viewport.offsetLeft}px`, '--clean-viewport-y': `${viewport.offsetTop}px` }" :aria-label="`用时 ${formatTime(state.elapsed)}，暂停游戏`" aria-haspopup="dialog" @keydown.space.stop @keydown.enter.stop @click="state.phase = 'paused'">{{ formatTime(state.elapsed) }}</button>
      <div v-if="!cleanHud" class="game-top">
    ...
    <footer v-if="!inGame && !inHome" class="bottom-bar"><span><i class="status-dot"/>{{ inAtlas ? '障碍物资料库' : state.error ? '场景加载失败' : state.ready ? '准备就绪' : '正在准备 3D 场景…' }}</span><span>{{ inAtlas ? '点选卡片 · 查看运转方式' : touchMode ? '横屏触控 · 支持键盘' : '键盘操作 · 推荐电脑浏览器' }}</span><span>LOCAL EDITION <b>01</b></span></footer>
    <div v-if="state.error && !inAtlas" class="error-banner" role="alert">{{ state.error }} <el-button size="small" @click="retrySceneLoad">重试加载</el-button></div>
